...
export function executeRelax(mesh: Mesh, json: any, isAction: boolean): Action | boolean {
    if (json.name !== _name_) {
        return false
    }
    ...
    const pos = relaxMesh(geom.attributes.position.array, geom.index.array, iter)
...
export class RelaxAction implements Action {
    ...
    constructor(private obj: Mesh, private iter: number) {
        this.geom = obj.geometry as BufferGeometry
        this.oldPos = cloneArray(this.geom.attributes.position.array)
        this.newPos = newArray(this.geom.attributes.position.array, this.geom.attributes.position.array.length)
    ...
    do() {
        this.geom.setAttribute('position', new BufferAttribute(new Float32Array(this.newPos), 3) )
    ...
    undo() {
        this.geom.setAttribute('position', new BufferAttribute(new Float32Array(this.oldPos), 3) )
        this.geom.attributes.position.needsUpdate = true
        this.geom.computeBoundingBox()
        this.geom.computeBoundingSphere()
    }
 ...
 */
function relaxMesh(positions: TypedArray | number[], indices: TypedArray | number[], iterations = 100, damp = 0.1) {
