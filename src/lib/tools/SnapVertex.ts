...
ToolFactory.register(
    'snapVertex',
    (params: ToolParameters) => new SnapVertexTool(params),
)
...
export class SnapVertexTool extends EventDispatcher implements Tool {
    ...
    private doAction(stack: ActionStack): void {
        ...
        if (
            this.currentIndex !== -1 &&
            this.targetID !== -1 &&
            this.currentIndex !== this.targetID
        ) {
            const to = new Vector3(
                pos.getX(this.targetID),
                pos.getY(this.targetID),
                pos.getZ(this.targetID),
            )
            this.actionStack.do(
                new SnapVertexAction(this.mesh, this.currentIndex, to),
            )
            this.currentIndex = -1
            this.targetID = -1
        }
